...
export function updateAwakeSettings(mode: ModeName, minutes: number, expireTime: string, displayOn: boolean) {
  ...
  if (!ptPath) {
    throw new Error("Could not find PowerToys installation.\nChecked paths:\n" + possiblePaths.join("\n"));
  }

  const settingsPath = path.join(localAppData, "Microsoft", "PowerToys", "Awake", "settings.json");
  ...
  if (mode === "Timed") {
    ...
    try {
      ...
      const expire = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h, m, s, 0);
